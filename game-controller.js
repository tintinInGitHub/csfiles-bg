...
class GameController {
    ...
    bindEvents() {
        ...
        document.getElementById('startMultiplayerGameBtn').addEventListener('click', () => this.startMultiplayerGame());
        ...
        document.getElementById('closeErrorBtn').addEventListener('click', () => this.gameUI.closeModal('errorModal'));

        // Murderer card selection modal events
        document.getElementById('confirmMurdererSelectionBtn').addEventListener('click', () => this.confirmMurdererSelection());
        document.getElementById('cancelMurdererSelectionBtn').addEventListener('click', () => this.gameUI.closeModal('murdererCardSelectionModal'));

        // Forensic Scientist scene selection modal events
        document.getElementById('confirmSceneSelectionBtn').addEventListener('click', () => this.confirmSceneSelection());
        document.getElementById('cancelSceneSelectionBtn').addEventListener('click', () => this.gameUI.closeModal('forensicSceneSelectionModal'));
    ...
    populateCardSelections(playerId) {
        ...
        this.gameUI.populateDropdown('clueCardSelect', clueCards.map(card => card.name));
        this.gameUI.populateDropdown('meanCardSelect', meanCards.map(card => card.name));
    }
    ...
    createCardSelectionInterface(murdererCards) {
        ...
        const clueSection = this.createCardSection('Clue Cards', murdererCards.clueCards, 'clue');
        container.appendChild(clueSection);

        // Create mean cards section
        const meanSection = this.createCardSection('Mean Cards', murdererCards.meanCards, 'mean');
    ...
    selectCard(cardName, type) {
        ...
        document.querySelectorAll(`.selectable-card.${type}-card.selected`).forEach(card => {
            card.classList.remove('selected');
        });

        // Add selection to clicked card
        const selectedCard = document.querySelector(`[data-card-name="${cardName}"][data-card-type="${type}"]`);
        if (selectedCard) {
    ...
    createSceneSelectionInterface(sceneTiles) {
        const container = document.getElementById('forensicSceneSelectionContainer');
        if (!container) return;
    ...
    selectSceneTile(tileText) {
        // Remove previous selection
        document.querySelectorAll('.selectable-scene-tile.selected').forEach(tile => {
            tile.classList.remove('selected');
        });

        // Add selection to clicked tile
        const selectedTile = document.querySelector(`[data-tile-text="${tileText}"]`);
        if (selectedTile) {
    ...
    showForensicRevealModal(forensicInfo) {
        const modal = GameUtils.createElement('div', {
            ...
            GameUtils.createElement('div', {
                ...
                GameUtils.createElement('p', {
                    textContent: 'The Forensic Scientist will guide the investigation and reveal clues during the game.',
    ...
    showGuessModal() {
        ...
        this.gameUI.populateDropdown('guessPlayerSelect', 
            activePlayers.map(p => ({ value: p.id, text: `Player ${p.id}` })));
        this.gameUI.populateDropdown('guessClueSelect', clueCards.map(card => card.name));
        this.gameUI.populateDropdown('guessMeanSelect', meanCards.map(card => card.name));
    ...
    submitGuess() {
        ...
        try {
            const result = this.gameCore.makeGuess(this.currentPlayer, playerId, clueCard, meanCard);
    ...
    confirmIdentity() {
        ...
        if (isNaN(playerId) || playerId < 1 || playerId > this.gameCore.gameState.playerCount) {
    ...
    updateGameUI() {
        ...
        this.gameUI.updatePhaseDisplay(gameState.currentPhase, gameState.currentRound);
    ...
    updateButtonStates() {
        ...
        const canGuess = currentPlayer && currentPlayer.hasBadge && gameState.currentPhase === 'investigation';
